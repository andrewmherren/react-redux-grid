...
var handleDrag = exports.handleDrag = function handleDrag(scope, columns, id, columnManager, store, nextColumnKey, stateKey, stateful, reactEvent) {
    ...
    var originalWidthPercent = parseFloat(window.getComputedStyle(columnNode).width, 10) * 100 / headerWidth;
    var programmedWidth = parseFloat(columnNode.style.width, 10);
    var totalWidth = parseFloat(columnNode.style.width, 10) + parseFloat(headerNextElementSibling.style.width, 10);

    var absWidth = computedWidth * 100;
    // stabilize a bit by limiting to one decimal point of precision
    var width = (absWidth * programmedWidth / originalWidthPercent * 10 | 0) / 10;
